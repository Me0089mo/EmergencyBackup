...
class CifradorAES_CFB(val applicationContext: Context) {
    ...
    fun cipherFile(path : Uri, fileName : String){
        ...
        applicationContext.contentResolver.openInputStream(path)?.use { reader ->
            ...
            do{
                ...
                if(byteOutStreamConv.isNotEmpty()) {
                    if(band == 1){
                        byteOutStreamConv.forEach { b -> print("$b ") }
                        print("\n")
                        band = 0
                    }
            ...
            zipOutStream.finish()
            finalizeCipher(byteOutReadStream.toByteArray())
            zipOutStream.close()
        }
    }
    ...
    private fun recoverKeys(){
        ...
        val user = File(applicationContext.filesDir, "userPubKey.pk").readBytes()
        val pubKeyU = X509EncodedKeySpec(user)
        userPublicKey = keyFac.generatePublic(pubKeyU)
        val server = File(applicationContext.filesDir, "userPubKey.pk").readBytes()
